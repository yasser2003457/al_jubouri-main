...
data,res = https.request("https://black-source.tk/al_jubouriTeAM/index.php?bn=info&id="..Id)
if res == 200 then
...
print("::al_jubouri::")
local RunBot = io.open("al_jubouri", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/al_jubouri
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr al_jubouri.lua
wget "https://raw.githubusercontent.com/Sourceal_jubouri/al_jubouri/master/al_jubouri.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./al_jubouri.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S al_jubouri -X kill
screen -S al_jubouri ./al_jubouri
done
]])
RunTs:close()
end
